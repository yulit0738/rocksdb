...
namespace rocksdb {
	namespace {
		...
		typedef InlineSkipList<const MemTableRep::KeyComparator&> KeyIndex;
		struct CuckooStep {
		...
		public:
			...
			public:
				...
				explicit Iterator(KeyIndex::Iterator* it,
					...
					const unsigned int& bucket_count_, HashCuckooRep* list);
		...
		public:
			...
			bool InsertIndexDataConcurrently(const char* internal_key);
			bool InsertIndexData(const char* internal_key);
		...
		CUCKOOCOLLISIONMOD:
			...
			if (QuickInsertConcurrently(key, user_key, bucket_ids, initial_hash_id) == false) {
				...
				if (FindCuckooPathConcurrently(key, user_key, &cuckoo_path_length, bucket_ids,
					...
					InsertIndexDataConcurrently(key);
					is_nearly_full_ = true;
					return;
				}
				else {
		...
		inline bool HashCuckooRep::InsertIndexDataConcurrently(const char* internal_key) {
			return KeyIndex_.InsertConcurrently(internal_key);
		}

		inline bool HashCuckooRep::InsertIndexData(const char* internal_key) {
			return KeyIndex_.Insert(internal_key);
		}
		...
		bool HashCuckooRep::QuickInsertConcurrently(const char* internal_key, const Slice& user_key,
			...
			while (true) {
				...
				if (is_key_update) {
					...
					InsertIndexDataConcurrently(internal_key);
					//}
					// 하고나면 끝
					return true;
				}
