...
namespace rocksdb {
	...
	public:
		...
		// Create a new YulInlineSkipList object that will use "cmp" for comparing
		...
		// skiplist object.
		explicit YulInlineSkipList(Comparator cmp, Allocator* allocator,
		...
		// REQUIRES: nothing that compares equal to key is currently in the list.
		// REQUIRES: no concurrent calls to any of inserts.
		Node* InsertWithHint(const char* key, void** hint);
		...
		Node* Insert(const char* key, Splice* splice, bool allow_partial_splice_fix);
		...
		public:
			...
			explicit Iterator(const YulInlineSkipList* list);

			// Change the underlying skiplist used for this iterator
			// This enables us not changing the iterator without deallocating
			// an old one and then allocating a new one
			void SetList(const YulInlineSkipList* list);
	...
	private:
		...
		// Returns the earliest node with a key >= key.
		// Return nullptr if there is no such node.
		Node* FindGreaterOrEqual(const char* key, Node* hint) const;
		...
		// No copying allowed
		YulInlineSkipList(const YulInlineSkipList&);
		YulInlineSkipList& operator=(const YulInlineSkipList&);
	};

	// Implementation details follow

	template <class Comparator>
	struct YulInlineSkipList<Comparator>::Splice {
	...
	template <class Comparator>
	struct YulInlineSkipList<Comparator>::Node {
	...
	template <class Comparator>
	inline YulInlineSkipList<Comparator>::Iterator::Iterator(
		const YulInlineSkipList* list) {
		SetList(list);
	}

	template <class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::SetList(
		const YulInlineSkipList* list) {
		list_ = list;
		node_ = nullptr;
	}
	...
	template <class Comparator>
	inline bool YulInlineSkipList<Comparator>::Iterator::Valid() const {
	...
	template <class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::Next() {
		assert(Valid());
		node_ = node_->Next(0);
	}

	template <class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::Prev() {
	...
	template <class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::Seek(const char* target) {
	...
	template<class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::Invalidate() {
		node_ = nullptr;
	}

	template <class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::SeekForPrev(
		const char* target) {
		Seek(target);
		if (!Valid()) {
			SeekToLast();
	...
	template <class Comparator>
	inline void YulInlineSkipList<Comparator>::Iterator::SeekToLast() {
	...
	template <class Comparator>
	int YulInlineSkipList<Comparator>::RandomHeight() {
	...
	template <class Comparator>
	bool YulInlineSkipList<Comparator>::KeyIsAfterNode(const char* key,
		Node* n) const {
	...
	typename YulInlineSkipList<Comparator>::Node*
		YulInlineSkipList<Comparator>::FindGreaterOrEqual(const char* key) const {
	...
	template <class Comparator>
	typename YulInlineSkipList<Comparator>::Node*
		YulInlineSkipList<Comparator>::FindGreaterOrEqual(const char* key, Node* hint) const {
		// Note: It looks like we could reduce duplication by implementing
		// this function as FindLessThan(key)->Next(0), but we wouldn't be able
		// to exit early on equality and the result wouldn't even be correct.
		// A concurrent insert might occur after FindLessThan(key) but before
	...
	template <class Comparator>
	typename YulInlineSkipList<Comparator>::Node*
		YulInlineSkipList<Comparator>::FindLessThan(const char* key, Node** prev) const {
		return FindLessThan(key, prev, head_, GetMaxHeight(), 0);
	}

	template <class Comparator>
	typename YulInlineSkipList<Comparator>::Node*
		YulInlineSkipList<Comparator>::FindLessThan(const char* key, Node** prev,
	...
	template <class Comparator>
	typename YulInlineSkipList<Comparator>::Node*
		YulInlineSkipList<Comparator>::FindLast() const {
		Node* x = head_;
		int level = GetMaxHeight() - 1;
		while (true) {
			Node* next = x->Next(level);
	...
	template <class Comparator>
	uint64_t YulInlineSkipList<Comparator>::EstimateCount(const char* key) const {
		uint64_t count = 0;
	...
	template <class Comparator>
	YulInlineSkipList<Comparator>::YulInlineSkipList(const Comparator cmp,
		Allocator* allocator,
		int32_t max_height,
	...
	template <class Comparator>
	char* YulInlineSkipList<Comparator>::AllocateKey(size_t key_size) {
	...
	template <class Comparator>
	typename YulInlineSkipList<Comparator>::Splice*
		YulInlineSkipList<Comparator>::AllocateSplice() {
	...
	template <class Comparator>
	template <bool prefetch_before>
	void YulInlineSkipList<Comparator>::FindSpliceForLevel(const char* key,
	...
	template <class Comparator>
	void YulInlineSkipList<Comparator>::RecomputeSpliceLevels(const char* key,
		Splice* splice,
		int recompute_level) {
	...
	template <class Comparator>
	bool YulInlineSkipList<Comparator>::Contains(const char* key) const {
	...
	template <class Comparator>
	void YulInlineSkipList<Comparator>::TEST_Validate() const {
