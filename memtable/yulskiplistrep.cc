...
namespace rocksdb {
	namespace {
		class YulSkipListRep : public MemTableRep {
			YulInlineSkipList<const MemTableRep::KeyComparator&> skip_list_;
		...
		public:
			explicit YulSkipListRep(const MemTableRep::KeyComparator& compare,
			...
			virtual void Get(const LookupKey& k, void* callback_args,
				...
				YulSkipListRep::Iterator iter(&skip_list_);
				Slice dummy_slice;
			...
			virtual ~YulSkipListRep() override { }

			// Iteration over the contents of a skip list
			class Iterator : public MemTableRep::Iterator {
				YulInlineSkipList<const MemTableRep::KeyComparator&>::Iterator iter_;

			public:
				// Initialize an iterator over the specified list.
				// The returned iterator is not valid.
				explicit Iterator(
					const YulInlineSkipList<const MemTableRep::KeyComparator&>* list)
					: iter_(list) {}
			...
			public:
				explicit LookaheadIterator(const YulSkipListRep& rep) :
			...
			private:
				const YulSkipListRep& rep_;
				YulInlineSkipList<const MemTableRep::KeyComparator&>::Iterator iter_;
				YulInlineSkipList<const MemTableRep::KeyComparator&>::Iterator prev_;
			};

			virtual MemTableRep::Iterator* GetIterator(Arena* arena = nullptr) override {
				if (lookahead_ > 0) {
					void *mem =
						arena ? arena->AllocateAligned(sizeof(YulSkipListRep::LookaheadIterator))
						: operator new(sizeof(YulSkipListRep::LookaheadIterator));
					return new (mem) YulSkipListRep::LookaheadIterator(*this);
				}
				else {
					void *mem =
						arena ? arena->AllocateAligned(sizeof(YulSkipListRep::Iterator))
						: operator new(sizeof(YulSkipListRep::Iterator));
					return new (mem) YulSkipListRep::Iterator(&skip_list_);
				}
			}
		};
	}

	MemTableRep* YulSkipListFactory::CreateMemTableRep(
		const MemTableRep::KeyComparator& compare, Allocator* allocator,
		const SliceTransform* transform, Logger* logger) {
		return new YulSkipListRep(compare, allocator, transform, lookahead_);
	}
